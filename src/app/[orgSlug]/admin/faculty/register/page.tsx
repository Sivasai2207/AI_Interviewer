...
import { createOrgUser, bulkCreateOrgFaculty, updateUserProfile } from "@/lib/firebase/firestore";
import { parseFacultyExcel, downloadFacultyTemplate } from "@/lib/excel";
import type { FacultyUploadRow, BulkUploadResult } from "@/types";
...
export default function RegisterFacultyPage() {
    ...
    // Bulk registration
    const [bulkData, setBulkData] = useState<FacultyUploadRowWithSelection[]>([]);
    ...
    const handleSingleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (result.success && result.uid) {
                // Update phone number separately as createOrgUser might not take it in base data for now
                // or we update UserProfile to support it and createOrgUser to pass it.
                // Assuming we need extra update:
                await updateUserProfile(result.uid, { phoneNumber: singleData.phoneNumber });

                setSingleSuccess(true);
                setSingleData({ name: "", email: "", phoneNumber: "", department: "", password: "" });
            } else {
                setSingleError(result.error || "Failed to create faculty member");
            }
        } catch (err: any) {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        console.log("[Register] Parsing Excel file:", file.name);
    ...
    const handleBulkSubmit = async () => {
        ...
        if (selectedRows.length === 0) {
            setParseError("Please select at least one member to register");
            return;
        }
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Add new faculty members to your organization
                    </p>
                </div>
            </div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
                ...
                <TabsContent value="single" className="mt-6">
                    <Card className="max-w-lg">
                        <CardHeader>
                            <CardTitle>Register Single Member</CardTitle>
                            <CardDescription>
                                Add one faculty member at a time
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            {singleError && (
                            ...
                            <form onSubmit={handleSingleSubmit} className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(value) =>
                                            setSingleData({ ...singleData, department: value })
                                        }
                                    >
                                        <SelectTrigger>
                        ...
                        {bulkData.length > 0 && (
                            <Card>
                                <CardHeader className="flex flex-row items-center justify-between">
                                    <div>
                                        <CardTitle>Preview ({bulkData.length} members)</CardTitle>
