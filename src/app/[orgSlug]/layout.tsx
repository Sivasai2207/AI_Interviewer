...
export default function OrgLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        if (orgSlug) {
            ...
            console.error("[OrgLayout] No orgSlug found in params");
            setIsLoading(false);
    ...
    useEffect(() => {
        ...
        if (!isLoading && organization && !authLoading) {
            ...
            if (!user) {
                console.log("[OrgLayout] User not logged in on protected route. Redirecting to:", `/${orgSlug}/login`);
                router.push(`/${orgSlug}/login`);
                return;
            }
            
            console.log("[OrgLayout] User authenticated:", {
                email: user.email,
                role,
                userOrgId,
                isPlatformOwner,
            ...
            if (isPlatformOwner) {
                console.log("[OrgLayout] Platform owner accessing org:", orgSlug);
            ...
            if (!userOrgId || userOrgId !== organization.id) {
                console.warn(`[OrgLayout] Access Denied. UserOrg (${userOrgId}) !== PageOrg (${organization.id})`);
            ...
            if (isAdminRoute && role !== "super_admin" && role !== "staff") {
                console.log("[OrgLayout] Non-admin accessing admin route. Redirecting to student portal.");
    ...
    const loadOrganization = async () => {
        console.log("[OrgLayout] Loading organization for slug:", orgSlug);
        try {
            const org = await getOrganizationBySlug(orgSlug);
            console.log("[OrgLayout] Loaded organization:", org);
            if (!org) {
                console.error("[OrgLayout] Organization not found for slug:", orgSlug);
                setError("Organization not found");
            } else if (org.status !== "active") {
                console.warn("[OrgLayout] Organization is suspended:", org);
                setError("This organization is currently suspended");
            } else {
                console.log("[OrgLayout] Organization loaded successfully:", org.name);
                setOrganization(org);
            }
        } catch (err) {
            console.error("[OrgLayout] Error loading organization:", err);
